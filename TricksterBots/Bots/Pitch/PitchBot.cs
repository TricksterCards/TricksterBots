...
namespace Trickster.Bots
{
    ...
    public class PitchBot : BaseBot<PitchOptions>
    {
        ...
        protected override Suit EffectiveSuit(Card c, Suit trumpSuit)
        {
            ...
            if (IsOffAceTrump && c.rank == Rank.Ace && Card.ColorOfSuit(c.suit) == Card.ColorOfSuit(trumpSuit))
                return trumpSuit;

            if (IsOffJackTrump && c.rank == Rank.Jack && Card.ColorOfSuit(c.suit) == Card.ColorOfSuit(trumpSuit))
                return trumpSuit;

            if (IsOffThreeTrump && c.rank == Rank.Three && Card.ColorOfSuit(c.suit) == Card.ColorOfSuit(trumpSuit))
                return trumpSuit;
        ...
        protected override int RankSort(Card c, Suit trumpSuit)
        {
            ...
            {
                case PitchVariation.FivePoint:
                    if (c.suit == trumpSuit && c.rank >= Rank.Jack)
                        return (int)c.rank + 1;
                    break;
                ...
                case PitchVariation.SixPoint:
                    if (c.suit == trumpSuit && c.rank >= Rank.Jack)
                        return (int)c.rank + 2;
                    if (c.rank == Rank.Jack && Card.ColorOfSuit(c.suit) == Card.ColorOfSuit(trumpSuit))
                        return (int)c.rank + 1;
                    if (c.suit == Suit.Joker)
                ...
                case PitchVariation.TenPoint:
                    ...
                    if (c.rank == Rank.Jack && Card.ColorOfSuit(c.suit) == Card.ColorOfSuit(trumpSuit))
                        return (int)c.rank + 2;
                    if (c.suit == Suit.Joker)
                        return (int)c.rank - (int)Rank.Low + (int)Rank.Jack;
                    break;

                case PitchVariation.ElevenPoint:
                    if (c.suit == trumpSuit && c.rank == Rank.Ace)
                        return (int)c.rank + 4;
                    if (c.rank == Rank.Ace && Card.ColorOfSuit(c.suit) == Card.ColorOfSuit(trumpSuit))
                        return (int)c.rank + 3;
                    if (c.suit == trumpSuit && c.rank >= Rank.Jack)
                        return (int)c.rank + 3;
                    if (c.rank == Rank.Jack && Card.ColorOfSuit(c.suit) == Card.ColorOfSuit(trumpSuit))
                ...
                case PitchVariation.ThirteenPoint:
                    ...
                    if (c.rank == Rank.Jack && Card.ColorOfSuit(c.suit) == Card.ColorOfSuit(trumpSuit))
                        return (int)c.rank + 3;
                    if (c.suit == Suit.Joker)
                        return (int)c.rank - (int)Rank.Low + (int)Rank.Jack + 1;
                    if (c.suit == trumpSuit && c.rank >= Rank.Three)
                        return (int)c.rank + 1;
                    break;
            }
