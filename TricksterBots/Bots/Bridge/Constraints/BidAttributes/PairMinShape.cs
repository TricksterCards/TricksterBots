...
namespace TricksterBots.Bots.Bridge
{
    public class PairHasMinShape : Constraint
    {
        ...
        public override bool Conforms(Bid bid, PositionState ps, HandSummary hs, PairAgreements pa)
        {
            ...
            (int Min, int Max) partnerShape = ps.Partner.PublicHandSummary.Suits[bid.SuitIfNot(_suit)].Shape;
    ...
    public class PairShowsMinShape : PairHasMinShape, IShowsState
    {
        ...
        void IShowsState.Update(Bid bid, PositionState ps, HandSummary hs, PairAgreements pa)
        {
           ...
           // Debug.Assert(newMin <= shape.Max);
            if (newMin > shape.Min) { 
                hs.Suits[bid.SuitIfNot(_suit)].Shape = (newMin, Math.Max(newMin, shape.Max));
            }
        }
    }
